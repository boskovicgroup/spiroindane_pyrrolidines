library(plyr)
# control names
names = c( "etoposide", "berberine HCl", "CA074 Me ester", "SB203580", "rapamycin", 
           "taxol", "latrunculine B", "cytochalasine B", "rotenone", "NPPB", 
           "tetrandrine","vincristine", "fenbendazole")

# Load the controls (3 replicates)
ctrl1 = read.csv('Ctrl1.csv')
...
rep1 = read.csv('Rep1.csv')
rep1$Name = rep1$Molecule
...
rep2 = read.csv('Rep2.csv')
rep2$Name = rep2$Molecule
...
rep3 = read.csv('Rep3.csv')
rep3$Name = rep3$Molecule
...
rep3.agg = aggregate(rep3, list(rep3$Molecule),mean)
...
actives = list()
actives = rbind(actives, rep1[rep1$Molecule == 'XK00001384-001' & rep1$Concentration == 50, ])
...
# Extract fingerprints of active compounds from replicate 2 of test compounds
actives2 = list()
actives2 = rbind(actives2, rep2[rep2$Molecule == 'XK00001384-001' & rep2$Concentration == 50, ])
actives2 = rbind(actives2, rep2[rep2$Molecule == 'XK00001401-001' & rep2$Concentration == 50, ])
actives2 = rbind(actives2, rep2[rep2$Molecule == 'XK00001401-001' & rep2$Concentration == 16.67, ])
actives2 = rbind(actives2, rep2[rep2$Molecule == 'XK00001401-001' & rep2$Concentration == 5.560, ])
actives2 = rbind(actives2, rep2[rep2$Molecule == 'XK00001387-001' & rep2$Concentration == 50, ])
actives2 = rbind(actives2, rep2[rep2$Molecule == 'XK00001386-001' & rep2$Concentration == 50, ])
...
ctrl1_plus_actives2 = rbind (ctrl1, actives2)
ctrl1_plus_actives2$Name = mapvalues(ctrl1_plus_actives2$Molecule, from=unique(ctrl1_plus_actives2$Molecule), to=names_with_actives)
ctrl1_plus_actives2.agg = aggregate(ctrl1_plus_actives2, list(ctrl1_plus_actives2$Name),mean)
rownames(ctrl1_plus_actives2.agg) = ctrl1_plus_actives2.agg$Group.1
nums <- unlist(lapply(ctrl1_plus_actives2.agg, is.numeric))  
ctrl1_plus_actives2.agg.num = ctrl1_plus_actives2.agg[,nums]
ctrl1_plus_actives2.agg.num.scaled <- scale(ctrl1_plus_actives2.agg.num)
ctrl1_plus_actives2.agg.cor = cor(t(ctrl1_plus_actives2.agg.num.scaled), use = 'pairwise.complete.obs')
plot(hclust(1 - as.dist(ctrl1_plus_actives2.agg.cor)), main = 'Actives from rep2')

ctrl1_plus_actives = rbind (ctrl1, actives)
...
plot(hclust(1 - as.dist(ctrl1_plus_actives.agg.cor)), main = 'Actives from rep1')
